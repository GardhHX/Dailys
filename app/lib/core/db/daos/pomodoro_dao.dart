...
class PomodoroDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<PomodoroSessionRow?> getById(String id) =>
      (select(pomodoroSession)..where((t) => t.id.equals(id)))
          .getSingleOrNull();
  ...
  Stream<List<PomodoroSessionRow>> watchCompletedFocusSessions(String userId) =>
  ...
  Stream<List<PomodoroSessionRow>> watchRecent(String userId,
          {int limit = 20}) =>
      (select(pomodoroSession)
  ...
  Future<int> getFocusStreak(String userId) async {
    final rows = await (select(pomodoroSession)
          ..where((t) =>
              t.userId.equals(userId) &
              t.isDeleted.equals(false) &
              t.status.equalsValue(PomodoroStatus.completed))
    ...
    for (final row in rows) {
      if (row.jenis == PomodoroJenis.istirahat_panjang) break;
      if (row.jenis == PomodoroJenis.fokus) count++;
    }
    return count;
  }
  ...
  Future<String> start({
    ...
    if (durasiMenit <= 0 || habitId != null) {
      throw PomodoroCommandException(
          'Invalid duration or unavailable Habit link');
    }
    await transaction(() async {
      if (await getInFlightSession(userId) != null) {
        throw PomodoroCommandException('Session already running');
      }
      if (tugasId != null) {
        final task = await (select(db.tugas)
              ..where((t) => t.id.equals(tugasId)))
            .getSingleOrNull();
        if (task == null ||
            task.userId != userId ||
            task.isDeleted ||
            task.archivedAt != null) {
          throw PomodoroCommandException('Task unavailable');
        }
      }
    ...
    await (update(pomodoroSession)..where((t) => t.id.equals(id))).write(
      PomodoroSessionCompanion(
        ...
        accumulatedPauseSeconds:
            Value(row.accumulatedPauseSeconds + addedPause),
        updatedAt: Value(ts),
      ),
    );
  }
  ...
  Future<String?> complete(
    ...
    return transaction(() async {
      final row = await _requireStatus(
          id, {PomodoroStatus.running, PomodoroStatus.paused});
      if (ts.isBefore(row.updatedAt)) {
        throw PomodoroCommandException('Timestamp command sebelum start_time.');
      }
      ...
      if (row.jenis == PomodoroJenis.fokus) {
        ...
        await into(activity).insert(
          ActivityCompanion.insert(
            ...
            endTime: Value(ts.millisecondsSinceEpoch ~/ 1000 >
                    row.startTime.millisecondsSinceEpoch ~/ 1000
                ? ts
                : null),
  ...
  Future<void> cancel(String id, {DateTime? now}) async {
    ...
    await (update(pomodoroSession)..where((t) => t.id.equals(id))).write(
      PomodoroSessionCompanion(
        ...
        accumulatedPauseSeconds: Value(accumulatedPause),
        actualSeconds:
            Value(_actualSeconds(row.startTime, ts, accumulatedPause)),
        updatedAt: Value(ts),
      ),
    );
  }
  ...
  int _actualSeconds(
      DateTime start, DateTime end, int accumulatedPauseSeconds) {
