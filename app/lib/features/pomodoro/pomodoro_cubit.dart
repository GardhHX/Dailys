...
class PomodoroCubit extends Cubit<PomodoroState> {
  ...
  Future<void> _init() async {
    ...
    try {
      ...
      if (settings == null) throw StateError('Settings unavailable');
      ...
      emit(state.copyWith(
          settings: settings,
          current: inFlight,
          clearCurrent: inFlight == null,
          focusStreakSinceLongBreak: streak,
          now: DateTime.now().toUtc(),
          loading: false));
      _settingsSub = _db.settingsDao.watchUserSettings(_userId).listen((s) {
        if (s != null && !isClosed) {
          emit(state.copyWith(settings: s, error: state.error));
        }
      }, onError: (_) {
        if (!isClosed) emit(state.copyWith(error: 'read'));
      });
    } catch (_) {
  ...
  Future<void> _command(Future<void> Function() action) async {
    if (state.busy || state.loading || state.error == 'read') return;
    emit(state.copyWith(busy: true));
    try {
      _pending = action();
      await _pending;
    } catch (_) {
      if (!isClosed) emit(state.copyWith(error: 'save'));
    } finally {
      _pending = null;
      if (!isClosed) emit(state.copyWith(busy: false, error: state.error));
    }
  }

  void _tick() {
    if (!isClosed) {
      emit(state.copyWith(now: DateTime.now().toUtc(), error: state.error));
    }
  }
  ...
  Future<int> _computeFocusStreak() => _db.pomodoroDao.getFocusStreak(_userId);
  ...
  Future<void> pause() => _command(() async {
  ...
  Future<void> resume() => _command(() async {
        final id = state.current?.id;
        if (id == null) return;
  ...
  Future<void> complete({String? judul}) => _command(() async {
        ...
        await _db.pomodoroDao.complete(
          ...
          activityCategoryId: isFocus
              ? DeterministicId.seedActivityCategory(_userId, 'personal')
              : null,
          occurrenceDate: isFocus
              ? LocalDate.fromInstant(current.startTime, _location).toYmd()
              : null,
        );
  ...
  Future<void> cancel() => _command(() async {
        final id = state.current?.id;
        if (id == null) return;
